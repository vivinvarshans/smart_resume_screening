...
class JobValidationError(JobServiceError):
    """Exception raised when job data validation fails."""

    def __init__(self, details: Optional[str] = None, job_id: Optional[str] = None):
        message = f"Job validation failed: {details}" if details else "Job validation failed"
        if job_id:
            message = f"{message} for job {job_id}"
        super().__init__(message)
        self.details = details
        self.job_id = job_id

class JobCreationError(JobServiceError):
...
class JobProcessingError(JobServiceError):
