...
class ResumeService:
    ...
    async def convert_and_store_resume(
        ...
        Returns:
            str: The generated resume_id

        Raises:
            ResumeValidationError: If file conversion or processing fails
        """
        temp_path = None
        try:
            ...
            if not result or not result.text_content:
                raise ResumeValidationError("No text content could be extracted from the file.")
            
            text_content = result.text_content

            # Store the resume and get the ID
            ...
            try:
                ...
                # Create a basic ProcessedResume record as fallback
                try:
                    await self._create_fallback_processed_resume(resume_id, text_content)
                    logger.info(f"Created fallback ProcessedResume for {resume_id}")
                except Exception as fallback_error:
                    logger.error(f"Failed to create fallback ProcessedResume: {str(fallback_error)}")
            ...
            raise ResumeValidationError(f"Resume processing failed: {error_msg}") from e
    ...
    async def _create_fallback_processed_resume(self, resume_id: str, resume_text: str) -> None:
        """
        Creates a basic ProcessedResume record with minimal data extraction
        when the full structured extraction fails.
        """
        # Extract basic keywords from the text (simple word extraction)
        words = resume_text.lower().split()
        # Filter for technical keywords (simple heuristic)
        tech_keywords = [
            word.strip('.,;:()[]{}') for word in words 
            if len(word) > 3 and any(tech in word.lower() for tech in [
                'python', 'java', 'javascript', 'react', 'node', 'sql', 'aws', 
                'docker', 'git', 'api', 'database', 'cloud', 'web', 'mobile',
                'data', 'machine', 'learning', 'ai', 'dev', 'engineer'
            ])
        ]
        # Remove duplicates and limit to 20 keywords
        tech_keywords = list(set(tech_keywords))[:20]
        
        if not tech_keywords:
            # If no tech keywords found, extract any meaningful words
            tech_keywords = [
                word.strip('.,;:()[]{}') for word in words 
        ...
        processed_resume = ProcessedResume(
            resume_id=resume_id,
            personal_data=json.dumps({"name": "Not extracted", "email": "", "phone": ""}),
            ...
            extracted_keywords=json.dumps({"extracted_keywords": tech_keywords}),
        )
