...
async def upload_resume(
    ...
    try:
        ...
        if not resume_id:
            logger.error("No resume ID was generated")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Failed to generate resume ID",
            )
            
        logger.info(f"Successfully processed resume with ID: {resume_id}")
        ...
        error_msg = str(e)
        logger.error(
            f"Error uploading resume: {error_msg} - traceback: {traceback.format_exc()}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def score_and_improve(
    ...
    try:
        ...
        logger.info(f"Improve request - resume_id: {resume_id}, job_id: {job_id}")
        ...
        error_detail = str(e)
        logger.error(f"Error in improve endpoint: {error_detail}")
